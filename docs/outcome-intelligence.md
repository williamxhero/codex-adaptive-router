...
`UserPromptSubmit` creates one task per user turn. `route_plan(task_ref=...)` returns the existing route. Asynchronous events are deduplicated and correlated by turn; append-only route events rebuild a missing ledger after a partial write. `Stop` records provisional evidence until objective verification or explicit user correction supplies quality.
